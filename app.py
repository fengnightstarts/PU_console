...
def main_menu(acc, user:dict, config:dict):
    list_type = "filter"
    all_list:list = []
    filter_list:list = []
    global target_list
    ...
    if target_list != []:
        ...
        list(["filter"], all_list, filter_list,target_list)
    while True:
        print("注意: 如果您没有保存活动列表, 则会自动抓取活动信息并list filter")
...
def add_act(all_list, filter_list, list_type, target_list, index):
    i = int(index)
    assert list_type in ["all", "filter"], f"add_act Error: list_type不在[all, filter]: {list_type}"
    if list_type == "all":
        if i > len(all_list) or i < 1:
            print("输入错误")
            return
