...
class Activity:
    ...
    def parse_datetime(self, date_str):
        assert isinstance(date_str, str) and date_str != None, f"Activity.parse_datetime Error:传入参数不为字符串或为空: {date_str}, {type(date_str)}"
        result = datetime.datetime.strptime(date_str, "%Y-%m-%d %H:%M:%S")
        assert isinstance(result, datetime.datetime), f"Activity.parse_datetime Error:从字符串转化datetime失败, 结果不为datetime: {date_str}, {result}, {type(result)}"
        return result
