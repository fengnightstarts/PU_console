...
class Account:
    ...
    def signup(self, act, token=None):
        ...
        assert isinstance(nowTime, datetime.datetime), f"account.signup Error: nowTime不为datetime: {nowTime}, {type(nowTime)}"
        ...
        if gap.total_seconds() > 0:
            nowTime = datetime.datetime.now()
            assert isinstance(nowTime, datetime.datetime), f"account.signup Error: nowTime不为datetime: {nowTime}, {type(nowTime)}"
            time.sleep((act.joinStartTime - nowTime).total_seconds()*random.random(0.9,1.5))
        i = 0
        while True:
            if self.flag[act.id]:
                break
            ...
            try:
                ...
                print(f"{act.name}第{i}次请求失败, {response.text}")
            if response.status_code == 200:
                print(f"活动 {act.name} 请求成功,{response.text} 请求时间:{datetime.datetime.now()}")
                if(response.text == '{"code":0,"message":"成功","data":{"msg":"PU君提示：报名成功，请留意活动签到时间哦~"}}'):
                    ...
                    try:
                        ...
                        self.lock.release()
                    break
                elif(response.text == '{"code":9405,"message":"您已报名，请勿重复操作","data":{}}'):
                    self.lock.acquire()
                    try:
                        self.flag[act.id] = True
                    finally:
                        self.lock.release()
                    break
                elif "请勿频繁点击" in response.text:
                    time.sleep(0.2)
            else:
                print(f"活动:{act.name} 第 {i} 次请求响应代码为{response.status_code}, 响应信息:{response.text}")
            if i < 5:
                time.sleep(0.1)
            elif i < 10:
